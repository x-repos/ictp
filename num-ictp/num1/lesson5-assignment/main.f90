...
contains
    ...
    function func(x) result(res)
        ...
        real(8) :: x
        real(8) :: res
        res = 15 * x**2 * gaussianfunc(x, real(0.25,8), real(-0.5,8)) + &
            13 * gaussianfunc(x, real(0.3,8), real(-1.5,8)) + &
            7 * gaussianfunc(x, real(1.,8), real(3.,8))
    end function func
...
contains
    ...
    subroutine kernel(x, xmin, xmax, n, filename)
        ...
        ! A = min(sigma, iqr/1.34). Normally sigma = 0.5
        a = iqr/1.34
        ! a = 1.0
        s = 0.9*a/n**(1.0/5)
        ngauss = 10000
...
program main
    ...
    call ecdf(x,n,'empiricalCDF.dat')
    ...
    call execute_command_line('gnuplot -p plot-main.plt')
    deallocate(x)
end program main    
